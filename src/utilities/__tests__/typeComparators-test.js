...
describe('typeComparators', () => {
  describe('isEqualType', () => {
    ...
    it('lists of same type are equal', () => {
      expect(
        isEqualType(wrapType(GraphQLInt, ']'), wrapType(GraphQLInt, ']')),
      ).to.equal(true);
    });

    it('lists is not equal to item', () => {
      expect(isEqualType(wrapType(GraphQLInt, ']'), GraphQLInt)).to.equal(
        false,
      );
    });
  ...
  describe('isTypeSubTypeOf', () => {
    ...
    it('non-null is subtype of nullable', () => {
      const schema = testSchema({ field: { type: GraphQLString } });
      expect(
        isTypeSubTypeOf(schema, wrapType(GraphQLInt, '!'), GraphQLInt),
      ).to.equal(true);
    });
    ...
    it('nullable is not subtype of non-null', () => {
      const schema = testSchema({ field: { type: GraphQLString } });
      expect(
        isTypeSubTypeOf(schema, GraphQLInt, wrapType(GraphQLInt, '!')),
      ).to.equal(false);
    });
